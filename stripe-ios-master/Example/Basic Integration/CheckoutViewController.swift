...
class CheckoutViewController: UIViewController {
    ...
    var stripePublishableKey = "pk_test_d06ETMjOEv3JaQ0F0ZEZA1Ie00Kgp3yKP0"
    ...
    var backendBaseURL: String? = "https://quiet-retreat-37404.herokuapp.com"
...
extension CheckoutViewController: STPPaymentContextDelegate {
    ...
    func paymentContext(_ paymentContext: STPPaymentContext, didFinishWith status: STPPaymentStatus, error: Error?) {
        ...
        let qrcode = QRCodeVC.generateQRImage(restID: "restaurantID", cardID: "giftcardID")!
        let QRCodeView = GiftCardView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: self.view.frame.height * 0.6), restName: "Anzio's Pizzeria", qrCode: qrcode, giftID: "giftcardID", width: 300)
        let giftCardImage = imageWithView(view: QRCodeView)
        self.sendMail(image: giftCardImage!)
    }
...
//MARK: SEND EMAIL WITH GIFT CODE
extension CheckoutViewController: MFMailComposeViewControllerDelegate {
    
    func sendMail(image: UIImage) {
      if MFMailComposeViewController.canSendMail() {
        let mail = MFMailComposeViewController()
        mail.mailComposeDelegate = self;
        mail.setToRecipients(["mohit.katyal417@gmail.com"])
        mail.setSubject("Here's a Gift Card to Anzio's Pizza")
        mail.setMessageBody("Hey!\nThanks for supporting your local business. Attached is a gift card for $20 that you can redeem in person!", isHTML: false)
        let imageData: Data = image.pngData()!
        mail.addAttachmentData(imageData as Data, mimeType: "image/png", fileName: "giftcard.png")
